...
실제 이상 대사가 선택되면 단계와 관계없이 해당 시청자의 전용 유형과 공용 글리치 풀을 함께 사용합니다. 예언·관찰·기억·모방·침입형은 각 원문으로 표시되고, `gli-*` 공용 붕괴형만 알 수 없는 문자로 변환됩니다. 모든 이상 채팅의 글꼴·굵기·배치·등장 효과는 일반 채팅과 같아서 문장 내용으로 판별해야 합니다. 연결 없음 상태에서는 별도의 화면 오염 효과가 적용됩니다.
...
- CSS의 `.open`, `.is-weak`, `.corrupted-chat-hit`, `.interference-*` 등의 클래스는 JavaScript 상태와 연결되어 있습니다.
