...
실제 이상 대사가 선택되면 단계와 관계없이 해당 시청자의 전용 유형과 공용 글리치 풀을 함께 사용합니다. 예언·관찰·기억·모방·침입형은 각 원문과 유형별 색으로 표시되고, `gli-*` 공용 붕괴형만 여러 줄의 알 수 없는 문자로 변환됩니다. 연결 없음 상태의 전체 채팅도 붕괴형과 같은 난독화 표시를 사용합니다.
