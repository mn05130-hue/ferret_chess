...
- 3단계 `약함`: 2초가 지난 뒤 빨간색, 재연결 가능
- 4단계 `끊김`: 총 4초 동안 재연결하지 않았을 때 검은색으로 전환
- 보통/약함에서 재연결: `+100점`
- 끊김 상태: 실제로는 정상인 채팅까지 화면에서 이상 채팅처럼 표시
- 좋음 상태에서 불필요하게 재연결: 체력 1 감소

끊김 상태는 재연결 버튼을 누를 때까지 유지됩니다. 이 상태에서 깨져 보이는 정상 시청자를 차단하면 정상 시청자 오판으로 판정되므로, 먼저 연결을 복구해야 합니다. 끊김 자체는 체력이나 점수를 감소시키지 않지만 스토리 모드의 오전 2시 결과 기록에는 남습니다.
...
실제 이상 대사는 `glitchChance` 확률에 따라 전용 유형 또는 공용 글리치 유형으로 먼저 나뉩니다. 예언·관찰·기억·모방·침입형은 각 원문으로 표시되고, `gli-*` 공용 붕괴형만 알 수 없는 문자로 변환됩니다. 모든 이상 채팅의 글꼴·굵기·배치·등장 효과는 일반 채팅과 같아서 문장 내용으로 판별해야 합니다. 연결 끊김 상태에서는 별도의 화면 오염 효과가 적용됩니다.
