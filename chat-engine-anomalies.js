...
const ANOMALY_LINES = Object.freeze({
  ...
  PROPHECY: [
    ...
    { id: "pro-omen",   level: 1, mode: "formal", needs: c => c.omen != null,
      make: c => `${c.seconds}초 뒤에 그들이 네 집에 도착한다.불끄지마 불끄면 걔가 방으로들어와 지금은 복도까지만 와있어 ${c.omen}` },
    { id: "pro-seen",   level: 1, mode: "formal", needs: c => c.omen != null,
  ...
  MIMIC: [
    { id: "mim-echo",   level: 1, mode: "raw", bypass: true, needs: c => Boolean(c.lastText),
      make: c => c.lastText +"니 오른쪽 아니 보지마 보지마 보지마 " },
    { id: "mim-same",   level: 1, mode: "casual", needs: c => Boolean(c.otherNick),
      make: c => `${c.otherNick}님은 곧 죽을 예정입니다. 나 아까부터 계속 보였는데 ` },
    { id: "mim-double", level: 1, mode: "raw", bypass: true, needs: c => Boolean(c.lastText),
      make: c => `${c.lastText}살려줘사렺 ${c.lastText}ㅁㅣ방에 불 한번만 껐다켜봐` },
    { id: "mim-gone",   level: 1, mode: "formal", needs: c => Boolean(c.otherNick),
      make: c => `${c.otherNick}님은 여기 없습니다. 찾지 마세요.뒤돌아보지말고 숨만 참아봐 니 숨소리 멈춰도 하나 남는지 들어봐` },
    { id: "mim-name",   level: 1, mode: "raw", needs: c => Boolean(c.otherNick),
      make: c => `${c.otherNick}${c.otherNick}${c.otherNick}` },
    { id: "mim-become", level: 1 , mode: "formal", needs: c => Boolean(c.otherNick),
      make: c => `${c.otherNick}님의 자리는 대체됩니다. 방에 불 한번만 껐다켜봐. 확실히 너 근처에 있는데 확인해보게` },
    { id: "mim-mine",   level: 1, mode: "casual",
